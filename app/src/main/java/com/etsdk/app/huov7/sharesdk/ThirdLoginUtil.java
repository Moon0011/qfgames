...
public class ThirdLoginUtil {
    ...
    public static void loginByThird(final int accountType,PlatformActionListener platformActionListener) {
        ...
        switch (accountType) {
            case LOGIN_QQ://qq
                platform = ShareSDK.getPlatform(AileApplication.getInstance(), QQ.NAME);
                break;
            case LOGIN_WX://微信
                platform = ShareSDK.getPlatform(AileApplication.getInstance(), Wechat.NAME);
                break;
            case LOGIN_XLWB://新浪
                platform = ShareSDK.getPlatform(AileApplication.getInstance(), SinaWeibo.NAME);
                break;
        }
        if (platform == null) {
            return;
