...
public class GameDetailV2Activity extends ImmerseActivity {
    ...
    private String[] titleList = new String[]{"简介", "礼包", "资讯"};
    ...
    private void setupUI(Intent intent) {
        ...
        fragmentList.add(NewsListFragment.newInstance(null, gameId));
...
//                tvRate.setText((gameBean.getDiscount()*10)+"折");
                if(gameBean.getFirst_discount()>0){
...
//                tvRate.setText("返利"+(gameBean.getDiscount()*100)+"%");
                tvRate.setText("返利" + Math.round(gameBean.getDiscount() * 1000)/10f + "%");
            }
        }else{
